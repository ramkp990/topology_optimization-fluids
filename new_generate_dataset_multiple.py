...
BATCH_SIZE         = 100     # designs to collect per script execution
VOLUME_THRESHOLD   = 0.2    # upper bound for random target volume
CHECKPOINT_FILE    = "./data/new1/generation_checkpoint.json"
OUTPUT_DIR         = "./data/new1"
...
def main():
    ...
    used_bc_configs = set(
        ...
        for pair in ckpt["used_bc_configs"]
    )
    ...
    while (completed < BATCH_SIZE
        ...
        n_out = np.random.choice([1,1]) # now only 1 outlet to simplify things
        ...
        # Check if any inlet and outlet are on the same wall
        # forbid outlet being on same wall as ANY inlet
        outlet_walls = [p["wall"] for p in ports if p["type"] == "outlet"]
        inlet_walls  = [p["wall"] for p in ports if p["type"] == "inlet"]

        same_wall_found = any(o in inlet_walls for o in outlet_walls)

        if overlap_found or same_wall_found:
            print(f"  attempt {attempts:3d} | port overlap or same-wall detected — resampling")
            continue
        ...
        for it in range(MAX_ESO_ITERS):
            ...
            feasible, reason = is_feasible(
                ...
                vol_min=0.10, vol_max=VOLUME_THRESHOLD,
                dp_min=0.005, dp_max=5.0
            )
            ...
            if SAVE_ALL_FEASIBLE and it >= 20 and feasible:
        ...
        if best_density is not None:
            ...
            os.makedirs("./output/plots/new", exist_ok=True)
            plot_path = (
                f"./output/plots/new"
