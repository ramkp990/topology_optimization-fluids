...
def decode_and_evaluate(solutions, vae, bc_mask_tensor, masks, ports,
    ...
    for density_np in density_all:
        ...
        try:
            ...
            dp_val  = dp.item()
            volume_penalty = abs(volume - 0.20)  # penalize deviation from target volume
            fitness = dp_val + lambda_volume * volume_penalty
...
def run_cmaes(ports, vae_path="vae_best_new.pth", lambda_volume=0.0):
    ...
    es = cma.CMAEvolutionStrategy(
        [0.0] * LATENT_DIM,
        #np.random.uniform(-3, 3, size=LATENT_DIM).tolist(),
        SIGMA0,
        {
            'bounds':    BOUNDS,
            'popsize':   POPSIZE,
    ...
    while not es.stop():
        ...
        '''
        for z, fit in zip(solutions, fitnesses):
            if fit < 1e5:
                #feasible_zs.append(np.array(z))
                #feasiblr_fits.append(fit)
                feasible_zs.append(np.array(z))
                feasible_fits.append((fit, dp_values[z], vol_values[z]))  # store tuple
        '''
        # save fitnesses of all feasible solutions in this generation
        for z, fit, dp_val, vol_val in zip(solutions, fitnesses, dp_values, vol_values):
            if fit < 1e5:
                feasible_zs.append(np.array(z))
                feasiblr_fits.append(fit)  # store only fitness values
    ...
    return best_z, best_design, best_dp, best_vol, feasible_zs, feasiblr_fits
...
if __name__ == "__main__":
    ...
    best_z, best_design, best_dp, best_vol, feasible_zs, feasible_fits = run_cmaes(
        ports         = ports,
        vae_path      = args.vae_path,
        lambda_volume = args.lambda_volume,
    )
'''
    if feasible_fits is None or len(feasible_fits) == 0:
        seed_z, feasible_zs, feasible_fits = None, None, None

    bo_z, bo_design, bo_dp, bo_vol = run_bo(
    ports         = ports,
    vae_path      = "vae_best_new.pth",
    seed_z        = best_z,        # warm-starts around CMA-ES optimum
    #seed_z       = np.random.uniform(-3, 3, size=LATENT_DIM).tolist(),  # random start
    feasible_zs   = feasible_zs,    # all CMA-ES feasible designs for surrogate training
    feasible_fits  = feasible_fits, # corresponding fitnesses for surrogate training    
    lambda_volume = 0.3,
    n_iterations  = 10,
    )
'''