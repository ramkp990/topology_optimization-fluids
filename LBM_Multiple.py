...
fluid_dilated = F.max_pool2d(
    orifice_mask.float().unsqueeze(0).unsqueeze(0), # to protect from closing. safety buffer
...
# ─── correct graph-memory measurement (saved-tensor hooks) ──────────────────
def measure_graph_memory(forward_fn):
    ...
    def pack_hook(t):
        ...
        if ptr not in seen:
            seen.add(ptr)
            saved_bytes[0] += t.element_size() * t.nelement()
        return t
...
def main():
    ...
    for it in range(n_iterations):
        ...
        # ─── measure LBM graph memory EXACTLY, ONCE (first iteration) ───────
        if it == 0:
            _loss_probe, lbm_mb = measure_graph_memory(
                lambda: simulate(alpha_max_constant)
            )
            if lbm_mb is not None:
                print(f"  [graph-mem] ESO LBM graph saved tensors: {lbm_mb:.1f} MB")
            else:
                print(f"  [graph-mem] saved_tensors_hooks unavailable (old PyTorch)")
            del _loss_probe        # free the probe graph; real one built below


        # normal forward + timed backward (unchanged logic):
        pressure_drop = simulate(alpha_max_constant)
        t0 = time.perf_counter()
        pressure_drop.backward()
        print(f"  [time] ESO backward: {(time.perf_counter()-t0)*1000:.1f} ms")
